...
- [ ] Tasks 7–11 not yet started. Task 8 is the first task requiring tablet access (a spike to verify the notebook-detection approach) — SSH access to the tablet (`ssh-copy-id`, needs the root password read off the tablet's Settings → Help → Copyrights and licenses → GPLv3 Compliance screen) is still not set up and will block Task 8 until done.
