...
- [x] **Task 6** — pen primitive, animator, debug renderer, `write_cursive` API. Implementation done (commit `e02e8c9`). Its own golden-PNG inspection surfaced a real, Important-severity visual defect inherited from Task 4's `CONNECTION_ZONE_HALF_WIDTH = 165.0`: the join check only compared glyph exit/entry *height*, never the actual distance between the two points, so visually-unrelated glyphs sharing a height band got spliced together, producing hard straight-line artifacts.
  - **Fixed** (commit `d6f3260`): added a Euclidean gap-distance check alongside the Y-zone check (join requires both; threshold 1.0× `x_height_px`, derived from real measured gaps: confirmed-good joins ≤25.52px, confirmed-bad joins ≥30.06px). New regression test `distant_same_zone_join_does_not_splice_into_one_stroke` (real 'o'→'n' pair). All 23 `cursive::` tests pass.
  - **Diagnosis corrected during verification:** true before/after golden-PNG pixel diffing plus join-decision logging showed the fix genuinely resolves the "étais" (t→a, gap 49.19px) artifact — but the "Bonjour" and "m'appelle" marks are unchanged and were never bugs. Raw glyph path data in `fonts/EMSAllure.svg` shows `j` and `p` are each a single continuous subpath with a large intrinsic loop descender (`j` to font-space y=-416, `p` to y=-369) — the font's own authentic swash-style letterform, not a cross-glyph join artifact. The original "3 artifacts from 3 bad joins" diagnosis was 1-for-3 correct.
  - **Final reviewer verdict (independently re-verified, not just told): Approved.** The reviewer generated their own before/after comparison and confirmed the "étais" fix; confirmed "Bonjour"/"m'appelle" are font-intrinsic via the same raw glyph data; downgraded that from Important to Minor (consistent, legible, a font-choice question for later — not a defect in this branch's code); verified all 23 tests pass independently.
  - **Housekeeping:** the reviewer's own verification process (a naive `cp -R` of the worktree, not realizing git worktrees share `.git` metadata with the main checkout) staled one file's git index entry. Working tree was never affected (confirmed `git diff HEAD` was empty before touching anything); fixed with `git reset -- src/cursive/layout.rs`.
- [x] **Task 7** — spiral gesture detection (pure logic). Clean; reviewer independently re-derived the turning-angle geometry in Python to confirm correctness, not just test-passing.
- [x] **SSH access established.** `ssh-copy-id` with the password (reMarkable Settings → Help → Copyrights and licenses → GPLv3 Compliance), added a `~/.ssh/config` Host entry for `10.11.99.1` reusing an existing personal key. Plain `ssh root@10.11.99.1` now works passwordlessly.
- [x] **Task 8** — notebook (open-document) detection. **Major plan deviation, resolved on real hardware:** the plan's primary approach (scan `/proc/<xochitl-pid>/fd` for an open document fd) was tested live and disproven — inspected xochitl's complete fd table over SSH; it holds no fd into the document data directory, only into an unrelated analytics event-cache DB. Found and verified a better real signal instead: each `<uuid>.metadata` file is JSON with an app-maintained `lastOpened` field (epoch ms) — confirmed across 5 real documents on the tablet to vary sensibly (e.g. two notebooks opened 12.5s apart show `lastOpened` 12572ms apart; folders have no such field). Implemented `notebook.rs` against this corrected approach; review clean. **Not verified:** whether `lastOpened` updates the instant a document opens vs. only on close — flagged for Task 11 to observe empirically on real usage.
- [x] **Task 9** — config fields, prompts, tool schema. Clean; reviewer specifically cross-checked `Config`/`Args` field-name alignment character-for-character (the one thing that would silently break `figment`'s name-based merge with no compile error).
- [x] **Task 10** — wire the tool, gesture trigger, prompt selection. The biggest integration task: `SpiralWatcher` appended to `gesture.rs` (Task 7's pure logic untouched — 0 deletions in that hunk), `gesture_trigger_task`/`GestureAnchor` added to `coordinator.rs`, `write_cursive` registered and `gesture_handle` spawned in `main.rs`. Review found 2 real Important bugs, both fixed and independently re-verified:
  1. `write_cursive`'s drawing omitted the fineliner tool-switch `draw_svg` already does — real risk of the diary's reply rendering with the wrong tool, or **erasing content** if the eraser was last selected. Fixed: wrapped with the same `select_fineliner`/`restore_tool` pattern.
  2. `--prompt general.json` was indistinguishable from the unset default (both were the same literal string) — silently blocked the one filename a user would naturally reach for to force the legacy `draw_text`/`draw_svg` experience back. Fixed: `Config.prompt`/`Args.prompt` changed from `String` to `Option<String>` (`None` = unset → notebook-aware selection; `Some(path)` = explicit override, including `general.json` itself).
  - Also confirmed on real hardware during this task (not just assumed): `/proc/bus/input/devices` shows `event1` = "Wacom I2C Digitizer" (pen), `event2` = "pt_mt" (touchscreen) — validates both the pre-existing `pen.rs`/`touch.rs` device paths and the new `SpiralWatcher`'s device path.
  - Minor items accepted as known brief-level tradeoffs, not fixed: a stale-anchor race if a second spiral is drawn while the first question is still awaiting its LLM response (single shared anchor cell, not per-request); `TriggerEvent::SpiralGesture`'s payload is dead code (the anchor actually flows through the separate mutex); a misleading shutdown-log message; synchronous file I/O inside the async `detect_open_document()` call.
  - 35/35 tests pass throughout, no regressions.
- [x] **Task 11 (largely complete as of 2026-07-18)** — on-device validation became an extended live-iteration phase with the user testing on real hardware; nearly every subsystem was revised based on observed failures. The plan's Step 3 (spiral calibration) is obsolete: the trigger design changed twice. Durable record of what on-device reality forced:
  1. **Trigger redesign, twice.** Spiral recognition never fired reliably on real handwriting → replaced with double-tap → false-fired on French accent marks (geometrically identical to deliberate taps) → triple-tap → still raced real writing → **final design: idle-timeout auto-trigger** (`ee44701`): 3.5s of pen inactivity after ≥1 non-tap stroke answers automatically, no gesture at all. Tap-shaped strokes (toolbar interactions done with the pen — observed arming the trigger with a sidebar-strip bbox — plus stray punctuation) delay but never arm it (`dff3f5d`); eraser strokes likewise.
  2. **CRITICAL incident: real data loss.** Task 10's review fix #1 (fineliner tool-switch before drawing) was itself calibrated for a Paper Pro's palette layout — on the RM2 those coordinates landed on the **Eraser**, and the first real end-to-end test erased the user's entire notebook page, unrecoverably. Full recalibration of `touch.rs` against RM2 3.27.3.0 with every coordinate screenshot-verified before hardcoding; `PenTool`/restore-tool removed entirely; `write_cursive` now deliberately draws with whatever tool the user has selected (no switching). Protocol since: no unverified UI coordinates, ever.
  3. **Question isolation for the vision model.** Haiku (default model, cost-chosen — note clap's `default_value` silently overrode `config.rs`'s default until `7546d74`) re-answered old questions on multi-question pages. Fixed by ground truth from pen events: the idle watcher accumulates the newest question's stroke bbox; `processing_task` crops that band and sends it as a second image ("the question to answer"), full page kept for history (`f4851c7`), anchor position appended to the prompt as text.
  4. **Placement.** The model's own y-guesses were unusable (answers landed above questions); placement now derives from the measured stroke anchor + fixed 70px clearance, x clamped ≥90 (toolbar occlusion). Progress dots follow the anchor too.
  5. **Page overflow → closed-loop scrolling** (`8e667d3`, spec §Error handling's deferred truncation concern, resolved differently): ink past y=1024 is silently lost (confirmed). Answers that would clip now dry-run the layout, then fling-scroll the canvas up and measure the real shift by screenshot ink-profile correlation (flings have momentum: fast 100px drag ≈ 260px, 200px ≈ 800px, slow drags ≈ 0 — calibrated live), drawing at the corrected y. Zero-shift pages (non-continuous) fall back to clipped drawing.
  6. **Input-stream reliability.** The pen evdev stream goes permanently silent after heavy simulated-write bursts (and once after a page switch) — likely lost epoll readiness. Mitigations: reopen after every drawing pass, on read errors, and proactively after 30s of total event silence; plus a `DrawingInProgress` flag so the watcher ignores the diary's own strokes (self-triggering was observed live).
  7. **Typography tuning from screenshots:** x-height 28→14px, word spacing 0.6→1.2, line spacing 2.2→3.2 (descender collisions), Latin-1 punctuation substitution (em-dash/curly quotes/ellipsis were silently dropped by the font), "Thinking" label removed (dots kept).
  - Steps 5 (diary-notebook persona live check) and 6 (mid-drawing cancellation) remain unvalidated on device; unplugged-operation test in progress by the user. Upstream note: fork has drifted ~4.5k lines from awwaiid/ghostwriter and `touch.rs` is now RM2-only — decision taken **not** to PR this branch upstream (cherry-pick device-neutral fixes only, e.g. the `processing_task` deadlock fix).

Full task-by-task detail (findings, fixes, reviewer verdicts) is in `.superpowers/sdd/progress.md`, which is gitignored (local scratch, not pushed) — this summary is the durable record. That ledger and this worktree (`.claude/worktrees/tom-riddle-diary`, branch `worktree-tom-riddle-diary`, tracking `origin`'s `tom-riddle-diary` branch / PR #1) are both still on disk — resuming from here (rather than re-cloning) preserves the local scratch ledger too, though this doc is sufficient on its own.
