...
- [ ] **Task 6** — pen primitive, animator, debug renderer, `write_cursive` API. Implementation and its own tests are done and reviewer-approved (commit `e02e8c9`), but the reviewer's golden-PNG inspection surfaced a real, Important-severity visual defect inherited from Task 4's `CONNECTION_ZONE_HALF_WIDTH = 165.0`: the join check only compares glyph exit/entry *height*, never the actual distance between the two points, so visually-unrelated glyphs that happen to share a height band get spliced together — producing hard straight-line artifacts across "Bonjour", "m'appelle", and "étais" in the golden sentence.
  - **Fix committed and pushed** (commit `d6f3260`): added a Euclidean gap-distance check alongside the Y-zone check (join requires both; threshold 1.0× `x_height_px`, derived from real measured gaps: confirmed-good joins ≤25.52px, confirmed-bad joins ≥30.06px). New regression test `distant_same_zone_join_does_not_splice_into_one_stroke` (real 'o'→'n' pair). All 23 `cursive::` tests pass.
  - **Verified, with a correction to the original diagnosis:** generating true before/after golden PNGs and diffing them pixel-by-pixel, plus temporary debug-logging every join decision in the golden sentence, showed the fix genuinely resolves the "étais" (t→a, gap 49.19px) artifact — but the "Bonjour" and "m'appelle" marks are **unchanged and are not bugs**. Pulled the raw glyph path data for `j` and `p` directly from `fonts/EMSAllure.svg`: both are a single continuous subpath with a large loop descender (`j` dips to font-space y=-416, `p` to y=-369) — this is the font's own authentic swash-style letterform, not a cross-glyph join artifact. The original implementer's and reviewer's diagnosis ("3 artifacts from 3 bad joins") was 1-for-3 correct; the fix targets the one real bug.
  - **Status: awaiting final reviewer sign-off.** Sent the corrected diagnosis + verification evidence back to Task 6's reviewer for an independent read on (a) the fix's own code/test quality, and (b) whether the "Bonjour"/"m'appelle" letterforms should be considered acceptable now that they're understood to be intentional font design. Work paused before that response arrived — **next session should check for the reviewer's reply first** (or re-review from scratch if the async response was lost) before marking Task 6 complete.
